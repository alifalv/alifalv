...
/**
 * 类型描述:封装一些与web相关公共逻辑实现 </br>创建时期: 2014年12月27日
 * 
 * @author hyq
 */
public class WebUtils {
	...
	public static String responseMsg(String msg,int code,long logId) {
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			
			map.put("code", code);
			map.put("msg", msg);
			map.put("errorId", logId);
			JsonConfig jsonConfig = new JsonConfig();
			final JsonDateValueProcessor dateValueProcessor = new JsonDateValueProcessor();
			jsonConfig.registerJsonValueProcessor(Timestamp.class,
					dateValueProcessor);
			...
			long eid = ExceptionLogger.writeLog(e, WebUtils.class);
			return "{\"code\":-1,\"msg:\":\"服务器异常\",\"errorId\":" + eid + "}";
		}
	}
	
	
	public static String responseData(String msg,int code,Object data) {
		try {
			Map<String, Object> map = new HashMap<String, Object>();
	...
	public static String responseMsg(String msg,int code) {
		try {
			Map<String, Object> map = new HashMap<String, Object>();
	...
	// 增加web端返回的数据格式，为了配合bootstrap框架返回 start
	public static String webResponseData(int code, Object data, int total) {
		return webResponseJson(data, total, null, 1, 0);
	}

	public static String webResponseData(Object data, int total) {
		return webResponseJson(data, total, null, 1, 0);
	}
	 ...
	 * @return
	 */
	public static String webResponsePage(List data) {
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("code", 1);
			...
			if(data!=null&&data.size()>0){
				...
				if(data.get(data.size()-1) instanceof Paging){
					
					Paging p = (Paging) data.get(data.size()-1) ;
					
					map.put("records", p.getTotalCount());
					map.put("page", p.getPageNo());
	 ...
	 * @author huangzenghai
	 * @param data 数据
	 * @param model 适配api和后台管理
	 * @return
	 */
	public static String webResponsePage(List data,int model) {
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			if(model == 3) {
				//如果 等于3，适配阿里的管理端表格控件；
				...
				Integer sEcho = null;
				Integer draw = null;
				Integer sColumns = null;
				Integer myprop = null;
				if(null != data && data.size() > 0) {
					if(data.get(data.size()-1) instanceof Paging){
						Paging p = (Paging) data.get(data.size()-1) ;
						iTotalRecords = p.getTotalCount();
						iTotalDisplayRecords= p.getTotalCount();
						data.remove(data.size()-1);
						aaData = data;
					}
				}
				...
				map.put("iTotalDisplayRecords",iTotalDisplayRecords);
					...
					if(data!=null&&data.size()>0){
						...
						if(data.get(data.size()-1) instanceof Paging){
							...
							if(model == 2){
								map.put("rowCount", p.getTotalCount());
								map.put("start", p.getPageNo()*p.getPageSize());
								map.put("total", p.getTotalPage());
								data.remove(data.size()-1);
								map.put("listMap", data);
								map.put("limit", p.getPageSize());
								map.put("currentPage", p.getPageNo());
							}else{
								map.put("records", p.getTotalCount());
								map.put("page", p.getPageNo());
								map.put("total", p.getTotalPage());
	...
	public static String webResponseJson(Object data, int total, String error,
			int code, long logId) {
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("code", code);
	...
	static class JsonDateValueProcessor implements JsonValueProcessor {
		...
		private String format_date_time = "yyyy-MM-dd HH:mm:ss";
