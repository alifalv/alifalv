...
/**
 * 提供访问数据库的公共泛型接口； 该接口可以由hibernate,jpa,jdbc,mybatis实现； 创建时间：2012-7-26
 * 
 * @author yuqing
...
public interface Dao<T> {
	 ...
	 * @return 实体对象的集合<br/>
	 *         <font
	 *         color='red'>特别说明：</font>该查询条件下的记录总行数，会封装到一个Paing实例，放在List的最后；<br/>
	 ...
	 *            查询条件的属性名称集
	 * @param opFlags
	 *            操作符集，支持“=,<>,>=,<=,<,>,like"这几种，其他不支持;这几种操作符要求传给正确的整形常量值，说明如下：
	 *            <li>DAO.OP_EQUALS 等于,值为0; <li>DAO.OP_GREAT_THAN 大于,值为1; <li>
	 *            DAO.OP_LITTER_THAN 小于,值为2; <li>DAO.OP_GREAT_EQUAL 大于等于,值为10;
	 *            <li>DAO.OP_LITTER_EQUAL小于等于,值为20; <li>DAO.OP_LIKE 模糊查询,值为3;
	 * @param conditionFlag
	 *            条件关系标识 ,支持AND ,OR,常量值说明如下： <li>DAO.CONDITION_AND AND,值为0; <li>
	 *            DAO.CONDITION_OR OR,值为1;
	 * @param values
	 *            查询条件属性值集;
	 * @return 返回符合条件的对象集合;
	 */
	List<T> queryByPropertys(String[] propertyNames, int[] opFlags,
	 ...
	 * @param paging
	 *            分页对象实例
	 * @return 返回符合条件的对象LIST集合; <font
	 *         color='red'>特别说明：该查询条件下的记录总行数，会封装到一个Paing实例，放在List的最后;
	 ...
	 * @return 返回符合条件的对象集合;
	 */
	List<T> queryByProperty(String fieldName, int opFlag, Object value)
			throws Exception;
	 ...
	 * 
	 * @param sql
	 *            合法的DML及DDL的SQL；如果是在mybatis实现下，则该参数是sqlid;
	 * @param parameters
	 *            该更新操作所需要的参数列表； <li>如果是jdbc原始实现：则该参数列表是零到多个对象; <li>
	 *            如果是mybatis实现，则该参数列表应该组织成一个map，或一个T实例；
	 * @return 返回更新所影响的行数；
	 * @exception 如果执行失败
	 */
	 ...
	 * 
	 * @param sql
	 *            合法的查询SQL；如果是在mybatis实现下，则该参数是sqlid
	 * @param parameters
	 *            该查询操作所需要的参数列表； <li>如果是jdbc原始实现：则该参数列表是零到多个对象; <li>
	 *            如果是mybatis实现， 则该参数列表应该组织成一个map，或一个实体类实例；
	 * @return 果一返回查询的结果List实例；list集合中的每个元素是查询结行数据， <br/>
	 *         行数据一般是一个(列名:值)的map
	 * @exception 如果执行失败
	 */
	 ...
	 * 
	 * @param sql
	 *            合法的查询SQL；如果是在mybatis实现下，则该参数是sql的映射id
	 * @param paging
	 *            分页信息实例
	 * @param parameters
	 *            该查询操作所需要的参数列表； <li>如果是jdbc原始实现：则该参数列表是零到多个对象; <li>
	 *            如果是mybatis实现， 则该参数列表应该组织成一个map，或一个实体实例；
	 * @return 返回查询的结果List实例；list集合中的每个元素是查询结果一行数据， <br/>
	 *         行数据一般是一个(列名:值)的map,<font
	 *         color='red'>特别说明：该查询条件下的记录总行数，会封装到一个Paing实例，放在List的最后;
	 *         控制层可以通过Paging.getTotaoCount()方法获得总行数值;</font>
	 * @exception 如果执行失败
	 */
	List executeQuery(String sql, Paging paging, Object... parameters)
			throws Exception;
