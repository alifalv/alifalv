 ...
 * 服务于DAO实现类，提供公共的逻辑实现
 * 
 * @author yuing </br> date 2014-0-06
 *
 */
class DaoUtils {
	...
	protected static Object stringToTypes(String fieldName, Object value,
	...
	protected static List<String> getAliasName(String sql) {
		...
		int pos = 0;
		for (int i = pos; sqltmp[pos].compareToIgnoreCase("FROM") != 0
				&& i < sqltmp.length; i++) {
	...
	/**
	 * 找到sql最外层查询的“FROM"的下标位置,并统计该下标前的”？“个数，将参数列表中对的参数删除； 查询出离select
	 * 最近的，且没有被()起来的第一个from的下标位置, *
	 * 
	 * @return
	 */
	...
	protected static int getFirstFrom(String sql) {
		/*
		 * 算法说明：
		 * 先计算出SQL的”FROM“单词的个数N，如果N为1，则第一个from就是要找的，否则按序从头遍历到FROM止；有几个from则遍历几次
		 * ：每次遍历如下操作： *
	 ...
	 * @param sql
	 *            待解析的SQL
	 * @param firstFromIndex
	 *            第一个From的下标;也就是通过getFirstFrom（）方法返回值
	 * @return
	 */
	protected static Object[] getCountFrontOfFirstForm(String sql,
			int firstFromIndex, Object[] params) {
		char[] sql_array = sql.toCharArray();
		int count = 0;
		// 统计在第一个FROM前的？的数量
