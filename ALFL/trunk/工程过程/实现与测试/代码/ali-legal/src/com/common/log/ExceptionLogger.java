 ...
 * 提供统一写异常日志的功能
 * 
 * @author yuqing </br>Date 2014-05-10
 */
...
public class ExceptionLogger {
	 ...
	 * 
	 * @param grad
	 *            日志等级，包括ExceptionLogger.DEBUG、INFO、WARN、ERROR、FATAL
	 * @param logContent
	 * @param exception
	 *            导常对象，如没有，可设置为null
	 * @param source
	 *            调用日志的类型
	 */
	public synchronized static long writeLog(int grad, String logContent,
			Exception exception, Class clazz) {
		long eid = 0 - System.currentTimeMillis();
		if (grad == ERROR || grad == FATAL)
			Logger.getLogger("").error(
					"LogId: @" + eid + "||||||" + logContent, exception);
		// LOG4J实现
		else
			switch (grad) {
			case 1:
				Logger.getLogger(clazz).debug(logContent, exception);
				break;
			case 2:
				Logger.getLogger(clazz).info(logContent, exception);
				break;
			case 3:
				Logger.getLogger(clazz).warn(logContent, exception);
				break;
			}
		return eid;
	}
