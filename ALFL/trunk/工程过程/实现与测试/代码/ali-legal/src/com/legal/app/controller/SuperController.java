...
public abstract class SuperController {
	...
	@ModelAttribute
	public void _init(HttpServletRequest request, HttpServletResponse response,
			Model model) {
	...
	private byte parseDeviceCode(HttpServletRequest request) {
		...
		} else if (value.indexOf("WINDOWS NT") >= 0
				|| value.indexOf("MAC OS") >= 0) {
	...
	protected void _exceptionHandle(Exception exception, PrintWriter out) {
		...
		if (exception instanceof BusinessException) {
			out.print(WebUtils.responseError(exception.getMessage(),
					((BusinessException) exception).getError(), errorLogId));
		} else {
			if (model.equals("DEBUG"))
				out.print(WebUtils.responseError(
						"系统发生异常:" + exception.toString(), -100, errorLogId));
			else
