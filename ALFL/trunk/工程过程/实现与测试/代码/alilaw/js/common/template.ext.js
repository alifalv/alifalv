...
define(function(require, exports, module) {
    ...
    window.template.defaults.imports.fromaUserName = function(value) {
        var faomaname = value.substring(0, 1);
        faomaname += "**"
        return faomaname;
    };
    ...
    //显示详情页图片
    window.template.defaults.imports.adaptionImgs = function(value) {
        var html = '';
        if (value) {
            var splitArr = value.split(',');
            for (var k in splitArr) {
                // html += '<li style="width: 120%;height: 100px;"><img src="' + createImgUrl(splitArr[k]) + '" class="hidden"/><img src="' + createImgUrl(splitArr[k]) + '" alt=""/></li>';
                html += '<li class="liImagDemo"><img imgzoom src="' + createImgUrl(splitArr[k]) + '" alt="" class="ui-bg-gray" style="min-height: 100px"></li>';
    ...
    window.template.defaults.imports.getArticleUrlByColumnCode = function(columnCode, articleId, userId) {
        if (columnCode == '0') { //百姓吐槽
            ...
            return constants.viewUrl + 'articles_commoninfo.html?articleId=' + articleId;
        ...
        }else if (columnCode == '16') { //个人主页
            return constants.viewUrl + 'home_index.html?userId=' + userId;
