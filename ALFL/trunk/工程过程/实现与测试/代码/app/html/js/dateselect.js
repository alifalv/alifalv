...
function dataList() {
	...
	// var date = sev_y + '-0' +sev_m + '-' + sev_d;
	// var date = sev_y + '-' +sev_m + '-' + sev_d;
	var date ;
	if(sev_m < 10){
		 date = sev_y + '-0' +sev_m + '-' + sev_d;
	}else {
		 date = sev_y + '-' +sev_m + '-' + sev_d;
	}
    // 获取用户信息
    var MAX = 1000;
    ...
    rest.post({
        ...
        data: {
            ...
            gregorianCalendar : date,//时间 (格式为：2018-10 )
            content : ''//内容
        },
        success: function (result) {
            ...
            if(result.rows && result.rows.length > 0) {
                for(var i=0;i< result.rows.length;i++) {
                    html += ('<li><p>' + result.rows[i].content + '</p><span>' + result.rows[i].gregorianCalendar + '</span></li>');
                }
			} else {

			}
			$('.j-datelist').html(html)
        }
    });
}
