var vm = new Vue({
    ...
    mounted: function(){
        ...
        rest.post({
            ...
            success: function (result) {
                if(result.data.declareImgs) {
                    result.data.declareImgs = result.data.declareImgs.split(',');
                }
                if(result.data.evidenceImgs) {
                    result.data.evidenceImgs = result.data.evidenceImgs.split(',');
                }
    ...
    methods:{
        ...
        daShang : function () {
            ...
            rest.post({
                ...
                data : {
                    ...
                    businessType :'阿里裁决',//业务类型 【传入中文】{"百姓吐槽","法律疑难问题与观点","公务员招聘","常用文书模板","文书制作知识","法律人信息","常见咨询问题","法律培训","法律法规","阿里裁决","我的风采","工作动态","成功案例","我的随笔","法律咨询","案件委托"};
                    currentId: _this.articleId//针对打赏数据的Id（【法律咨询】 咨询解答这一行的Id【阿里裁决】 阿里裁决的Id；【百姓吐槽】 百姓吐槽的Id）
                },
                success: function(result) {
        ...
        toList : function(func) {
            var _that = this;
            rest.post({
                url: 'reply/commonReplyList/{relativeId}/{businessType}/{pageNo}/{pageSize}',
                urlParams: {
