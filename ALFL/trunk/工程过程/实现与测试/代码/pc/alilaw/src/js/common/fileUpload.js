...
define(function(require,exports,module) {
    ...
    ;(function ($) {
        ...
        $.fn.extend({
            createFileUpload: function (opt, serverCallBack) {
                var imageNum = 5;
                ...
                var defaults = {
                    fileType: ["doc", "docx", "pdf", "jpg", "png"], // 上传文件的类型
                    fileSize: 1024 * 1024 * 10, // 上传文件的大小 10M
                    count: 0
                    // 计数器
                };

                if(null != opt.fileUrl && '' != opt.fileUrl && opt.fileUrl.length > 0) {
                    ...
                    if(opt.fileUrl instanceof Array) {
                        imgArr = imgArr.concat(opt.fileUrl);
                    } else if(typeof opt.fileUrl == 'string') {
                        imgArr.push(opt.fileUrl)
                    }
                    for(var i = 0; i < imgArr.length; i++) {
                        ...
                        var $img = $("<a class='up-img up-opcity ui-height-100p ui-block file-look' target='_blank' title='单击查看'></a>");
                        $img.attr("href", utils.createFileUrl(imgArr[i]));
                        ...
                        if (newStr.split(".")[0] != null) {
                            ...
                            if (jQuery.inArray(type, defaults.fileType) > -1) {
                                ...
                                if (file.size >= defaults.fileSize) {
                                    C.msg.fail('文件大小"' + file.name + '"超出10M限制！');
                                } else {
                                    arrFiles.push(file);
                                }
                            } else {
