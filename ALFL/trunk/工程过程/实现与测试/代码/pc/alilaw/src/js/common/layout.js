...
define(function(require,exports,module) {
    ...
    if(utils.hasClass('.user_menu')) {
        ...
        if(utils.hasClass('.user_reply')) {
            if(userInfo.userType == '2') {
                rest.post({
                    url: 'accessLog/listAccessLog',//112.咨询师最近访客和统计个数;9.20
                    data : {
            ...
            rest.post({
                url: 'advice/myReplyRecently/{userId}',//68.	个人咨询者账户中心-最近回复；9.20
                urlParams : {
                    userId : userInfo.userId
                },
                success: function (result) {
        ...
        if(utils.hasClass('.user_count')) {
            rest.post({
                url: 'article/counts/{userId}',//111.咨询师/咨询/关注/收藏数统计;9.19
                urlParams : {
                    userId : userInfo.userId
                },
                success: function (result) {
                    ...
                    $('#jtsgamfls_btn').on('click', function() {
                        ...
                        utils.loadHtml(_$jtsgamfls_div, '../tmpl/jtsgamfls_dialog.html', function() {
                            ...
                            _$jtsgamfls_div.find('.js-ok-btn').on('click', function() {
                                ...
                                isSending = true;
                                rest.post({
                                    url: 'user/freeCaseDeclare',//78.	个人咨询者账户中心-免费申请 交通事故案件委托；9.14完成
                                    data : {
                                        userId : userInfo.userId
                                    },
                                    success: function (result) {
    ...
    function addBrowse(userId, counselorId) {
        rest.post({
            url: 'user/addBrowse',//6.	咨询师个人信息/咨询者/企业咨询者查询
            data: {
                userId: userId,//用户Id
    ...
    if(utils.hasClass('.home_right')
        ...
        rest.post({
            url: 'user/userInfo/{userId}',//6.	咨询师个人信息/咨询者/企业咨询者查询
            urlParams : {
                userId : userId
            },
            success: function (result) {
