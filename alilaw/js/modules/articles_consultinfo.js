...
define(function(require, exports, module) {
    ...
    //因涉及到优先前3名回复可获得赏金，律师不允许删除自己回复的贴
    ...
    function commonReply($form, replyId, func) {
        ...
        var cont = $('#replyContent').val().length;

        var obj_lis = document.getElementById("topDetil").getElementsByTagName("li");

        var dddd = obj_lis[0].innerText;
        if (dddd.substring(3, dddd.length - 1) != "0") {
            if (cont < 50) {
                alert('最少输入50个字符');
                return false;
            }
    ...
    function initReplyList() {
        ...
        $commonReplyList.on('click', '.js-replybox-btn', function() {
            ...
            if (authorUserId != _loginUserInfo.userId && _loginUserInfo.userType==1) {
                C.msg.fail('只限发布者回复');
                return false;
            }
            //清空留言box
